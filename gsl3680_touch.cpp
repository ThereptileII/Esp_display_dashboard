...
static i2c_master_dev_handle_t s_probe_dev = nullptr;
...
bool gsl3680_touch::begin()
{
    ...
    // Bring up the esp-idf v5 I2C master bus that the esp_lcd IO layer expects
    if (s_i2c_bus) {
    ...
    err = i2c_master_bus_add_device(s_i2c_bus, &dev_cfg, &s_probe_dev);
    if (err != ESP_OK) {
        Serial.printf("[gsl3680] ERROR: failed to create probe device: %s\n", esp_err_to_name(err));
        return false;
    }
    err = i2c_master_transmit(s_probe_dev, &ping_reg, 1, 50 / portTICK_PERIOD_MS);
    i2c_master_bus_rm_device(s_probe_dev);
    s_probe_dev = nullptr;
    if (err != ESP_OK) {
